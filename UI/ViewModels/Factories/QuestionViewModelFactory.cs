...
public class QuestionViewModelFactory(Evaluator evaluator, IErrorHandlerService errorHandler, IStorageService storageService) : IViewModelFactory {
    ...
    public QuestionFormBaseVM NewQuestionFormVm(int index, AbstractQuestion? question = null) => index switch {
        CreateStyleQuestionIndex => new CreateStyleQuestionFormViewModel(errorHandler, storageService, question),
        ParagraphApplyStyleQuestionIndex => new ParagraphApplyStyleQuestionFormViewModel(errorHandler, storageService, question),
